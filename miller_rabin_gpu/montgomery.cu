        ...
        int n_limbs)
{
    ...
    if (threadIdx.x == 0) { match_a = 1; match_b = 1; }
    __syncthreads();
    ...
    for (int j = (int)threadIdx.x; j < n_limbs; j += (int)blockDim.x) {
        if (rv[j] != ra[j]) atomicAnd(&match_a, 0);
        if (rv[j] != rb[j]) atomicAnd(&match_b, 0);
    }
    __syncthreads();
                                          ...
                                          int offset, int n_out, int n_src, int n_batch)
{
    int cand = blockIdx.y;
    int j    = blockIdx.x * blockDim.x + threadIdx.x;
    if (cand >= n_batch || j >= n_out) return;
    int s = j + offset;
...
{
    int cand = blockIdx.x;
    if (cand >= n_batch || threadIdx.x != 0) return;
    ...
    if (gcmp < 0) {
        ...
        for (int t = 0; t < n_tiles; t++) bin[t] = -1;
        return;
    }
        ...
        int n, int n_batch)
{
    ...
    if (tile_bin < 0) return;  // a < N, sem subtração
    ...
    int excl = (tid == 0) ? 2 : s_state[tid - 1];  // não correto ainda
...
{
    mpz_t N, R, Np; mpz_init(N); mpz_init(R); mpz_init(Np);
    limbs_to_mpz(N, N_lims, n);
    mpz_ui_pow_ui(R, 2, (unsigned long)(LIMB_BITS*n));
    if (!mpz_invert(Np, N, R)) throw std::runtime_error("N nao tem inverso mod R");
    mpz_sub(Np, R, Np);
    mpz_to_limbs(Np_out, n, Np);
    mpz_clear(N); mpz_clear(R); mpz_clear(Np);
}
    ...
    , ntt(n_limbs_, n_batch_)
{
    ...
    for (int i = 0; i < n_batch; i++) one_lims[i * n_limbs] = 1;
                                   ...
                                   std::vector<uint64_t>& out_all) const
{
    ...
    mpz_t xm, N, R, res; mpz_init(xm); mpz_init(N); mpz_init(R); mpz_init(res);
    for (int i = 0; i < n_batch; i++) {
        ...
        mpz_mul(res, xm, R); mpz_mod(res, res, N);
        mpz_to_limbs(out_all.data() + i*n_limbs, n_limbs, res);
    }
    mpz_clear(xm); mpz_clear(N); mpz_clear(R); mpz_clear(res);
}
...
void BatchMontCtx::from_mont_batch(const Data64* d_x, std::vector<uint64_t>& out_all) const
{
    ...
    mpz_init(xm); mpz_init(N); mpz_init(R); mpz_init(Rinv); mpz_init(res);
    for (int i = 0; i < n_batch; i++) {
    ...
    mpz_clear(xm); mpz_clear(N); mpz_clear(R); mpz_clear(Rinv); mpz_clear(res);
}
