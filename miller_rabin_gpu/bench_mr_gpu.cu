...
int main(int argc, char *argv[])
{
        ...
        for (int i = 1; i < argc; i++)
        {
                if (std::string(argv[i]) == "--test")
                        ...
                        show_report = true;
                else if (std::string(argv[i]) == "--progress")
                        show_progress = true;
                else if (std::string(argv[i]) == "--bench-ops")
                        run_bench = true;
                else if (std::string(argv[i]) == "--bench-ops-long")
                        run_bench_long = true;
        ...
        if (cands.empty())
        {
                fprintf(stderr, "Nenhum candidato.\n");
                return 1;
        }
        ...
        if (run_tests)
        {
                ...
                for (int i = 0; i < bsz_test; i++)
                        test_cands.push_back(&cands[i].N_cand);
        ...
        for (int b = 0; b < n_batches; b++)
        {
                ...
                for (int i = 0; i < bsz; i++)
                        N_cands.push_back(&cands[bstart + i].N_cand);
                ...
                if (survivors.empty())
                {
                        printf("Nenhum sobreviveu ao teste de N.\n");
                        continue;
                }
                ...
                for (int si : survivors)
                        revN_cands.push_back(&cands[bstart + si].revN_cand);
